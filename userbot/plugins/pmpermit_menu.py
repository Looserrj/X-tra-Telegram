    ...
    if not pmpermit_sql.is_approved(chat_id):
        ...
        if event.is_private:
         ...
         PM = ("`Hello. You are accessing the availabe menu of my master,`"
               ...
               "`3`. To Have a Serious Discussion .\n"
        ...
        async with borg.conversation(chat) as conv:
         ...
         if y == "1":
             ...
             if x == "1":
                 ...
                 await borg.send_message(chat, "** I will reply you when i get online.**")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
                 ...
                 await borg.send_message(chat, "`Do not send more messages and let my master see for himself if he wants to talk with you.`")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
